...
#define UPPER_HAND                  "Upperhand" // 손의 회전 정도 정보와 마커 정보를 갖고 오기 위한 Model ID , Streaming ID == 1
...
Wrist myWrist; // 손목 객체
Marker myMarker; // 핑거팁 표시용  marker 객체 . 현재는 싱글로 놔둠 (07.15)
Serial* sp = new Serial("\\\\.\\COM4"); // 시리얼 포트 번호 꼭 보고 바꿔줘야함 
...
{
    char* ptr = inptr; // 데이터를 받아올 string 변수 
...
char* UnpackMarkersetData(char* ptr, int major, int minor)
{
    ...
    for (int i = 0; i < nMarkerSets; i++)
    {
        ...
        //if(szName== UPPER_HAND)
            printf("Model Name       : %s\n", szName); // Rigidbody 인 경우 rigidbody 이름을 불러옴 
...
char* UnpackRigidBodyData(char* ptr, int major, int minor)
{
    ...
    for (int j = 0; j < nRigidBodies; j++)
    {
        ...
        Vector3 pos(x, y, z);
        Quternion orientation(qx, qy, qz, qw);

        printf("  RB: %3.1d ID : %3.1d\n", j, ID);
        
        if (ID == UPPER_HAND_ID) // 만약 손목이 감지되었다면 
...
// labeled markers - this includes all markers: Active, Passive, and 'unlabeled' (markers with no asset but a PointCloud ID)
    if (((major == 2) && (minor >= 3)) || (major > 2))
    {
        ...
        for (int j = 0; j < nLabeledMarkers; j++)
        {
            ...
            // 여기서 각 마커의 POS를 처리해줘야겠네. 그럼 처음 인식 받고 나서 ModelID랑 마커셋의 할당 ID 매칭해서 거기에 맞게
            // 데이터를 가져오도록 바꿔줘야겠구나. ssss
            ...
            */
            ...
            myMarker.SetMarkerPos(pos); // 마커의 위치를 업데이트 해줌 
           // Vector3 temp;
           // myMarker.GetMarkerPos(temp); 
            //cout << temp; // 테스트용으로 마커의 위치를 출력함 
